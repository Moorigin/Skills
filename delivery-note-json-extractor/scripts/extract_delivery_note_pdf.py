...
def normalize_text(value: Any) -> str:
    ...
    return (
        ...
        .translate(PDF_GLYPH_TRANSLATION)
        .replace("\u00a0", " ")
    )
...
def choose_delivery_date(
    text: str,
    labels: Iterable[str],
    *,
    allow_unlabeled_fallback: bool = True,
) -> str:
    ...
    if not allow_unlabeled_fallback:
        raise ExtractionError(f"未找到指定日期字段: {', '.join(label_values)}")
...
def extract_platform_skc(value: Any) -> str:
    ...
    match = re.search(r"YY[A-Z]*(\d+)", text)
...
def extract_prefixed_identifier(value: Any, prefix: str) -> str:
    match = re.search(rf"{re.escape(prefix)}\d{{8,}}(?:-\d+)?", compact(value), re.IGNORECASE)
    return match.group(0).upper() if match else ""


def extract_pb_fragment(value: Any) -> str:
    text = compact(value).upper()
    match = re.search(r"PB\d+(?:-\d*)?", text)
    if match:
        return match.group(0)
    return text if re.fullmatch(r"-?\d+", text) else ""


def join_identifier_fragments(left: str, right: str) -> str:
    prefix = compact(left).upper()
    suffix = compact(right).upper()
    if not prefix:
        return suffix
    if not suffix:
        return prefix
...
def resolve_pb_order(fragment: str, header_orders: Iterable[str]) -> str:
    value = extract_pb_fragment(fragment)
    if not value:
        return ""
    orders = [compact(order).upper() for order in header_orders if compact(order)]
    if value in orders:
        return value
    if value.startswith("PB"):
        candidates = [order for order in orders if order.startswith(value)]
    else:
        candidates = [order for order in orders if order.endswith(value)]
    if len(candidates) == 1:
        return candidates[0]
    if not orders and re.fullmatch(r"PB\d{8,}(?:-\d+)?", value):
        return value
    return ""


def is_order_fragment_only_row(
    row: list[str | None],
    order_index: int | None,
    quantity_index: int | None,
) -> bool:
    if order_index is None or parse_quantity(cell(row, quantity_index)) is not None:
        return False
    if not extract_pb_fragment(cell(row, order_index)):
        return False
    return all(not compact(value) for index, value in enumerate(row) if index != order_index)


def apply_pending_pb_fragment(document: dict[str, Any], fragment: str) -> str:
    combined = join_identifier_fragments(document.get("pending_order_fragment", ""), fragment)
    order = resolve_pb_order(combined, document.get("header_orders", []))
    if not order:
        document["pending_order_fragment"] = combined
        return ""

    pending_row = document.get("pending_row")
    if pending_row is not None:
        pending_row["order_number"] = order
        document["rows"].append(pending_row)
    document["pending_row"] = None
    document["pending_order_fragment"] = ""
...
def detect_layout(pages: list[PageData]) -> str:
    ...
    if all(token in collapsed for token in ("备货单号", "SKC货号", "属性集")):
        return "temu"
    if all(token in collapsed for token in ("订单号:POCY", "下单时间", "SKU货号")):
        return "tk"
...
def parse_tk(pages: list[PageData]) -> dict[str, Any]:
    all_text = "\n".join(page.text for page in pages)
    delivery_date = choose_delivery_date(
        all_text,
        ("下单时间",),
        allow_unlabeled_fallback=False,
    )
...
def parse_temu(pages: list[PageData]) -> dict[str, Any]:
    ...
    for group in groups:
        ...
        if not order_match:
            raise ExtractionError("TEMU 商品组缺少 WB 备货单号")
        order = order_match.group(1).upper()
...
def shein_header_orders(text: str) -> list[str]:
    ...
    values = re.findall(r"PB\d{8,}(?:-\d+)?", source, flags=re.IGNORECASE)
...
def parse_shein_packing(pages: list[PageData]) -> dict[str, Any]:
    ...
    for page_number, page in enumerate(pages, start=1):
        ...
        for table in page.tables:
            ...
            for row in table[1:]:
                ...
                if is_order_fragment_only_row(row, order_index, quantity_index):
                    apply_pending_pb_fragment(document, cell(row, order_index))
                    continue

                quantity = parse_quantity(cell(row, quantity_index))
                if quantity is None:
                    continue
                if document["pending_row"] is not None:
                    raise ExtractionError(
                        f"SHEIN 配货单 {shipment} 的跨页订单号残段未能在续页首行完成拼接"
                    )
                ...
                if order_index is not None:
                    fragment = extract_pb_fragment(cell(row, order_index))
                    if fragment:
                        order = resolve_pb_order(fragment, document["header_orders"])
                        if not order:
                            ...
                            document["pending_order_fragment"] = ""
                            apply_pending_pb_fragment(document, fragment)
                            continue
                    else:
                        order = document["last_order"]
    ...
    for document in documents.values():
        if document["pending_row"] is not None or document["pending_order_fragment"]:
            raise ExtractionError(
                f"SHEIN 配货单 {document['shipment']} 存在未解析的跨页订单号残段"
            )

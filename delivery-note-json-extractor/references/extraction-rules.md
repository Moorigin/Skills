...
Use this reference when validating a supported merged PDF or when the deterministic parser cannot complete a page safely.
...
2. Normalize Unicode compatibility characters and remove whitespace inside identifiers for matching. Keep table columns and document boundaries intact.
3. Extract `platform_skc` as the consecutive digits immediately after `YY`. Stop at the first non-digit regardless of whether the suffix is `NQL`, another known or unknown letter combination, punctuation, or the end of the code. Examples: `YY059NQL` -> `059`, `YY12345ABC` -> `12345`, and `YY88008` -> `88008`.
4. Remove only terminal size markers from colors, such as `-XXS`, `-XS`, `-S`, `-M`, `-L`, `-XL`, `-XXL`, `-3XL`, and `-均码`. Remove embedded codes such as `JC076` while preserving meaningful Chinese color words.
5. Merge by `(order_number, platform_skc, attribute_set)`. Preserve conflicting SKCs or colors as separate records.
6. Use printed totals only for reconciliation. Never add both detail rows and totals.
7. Require a single delivery date across one merged input file. Stop and report all conflicting dates instead of selecting the first.
...
- Use one unified parsing route for both table shapes:
  - When a row-level `订单号` column exists, rejoin its wrapped `PB...` cell and use it.
...
- Validate row orders against the full PB list in that FH header. Do not assign all rows in a multi-order table to the first header order.
...
- Use the PB value in the row-level `订单号` column.
